...
JOIN departamento AS d ON m.depid = d.id
JOIN pais AS p ON d.paisid = p.id;
```
...
```sql
SELECT c.nombre,m.nombre,d.nombre,p.nombre
FROM clientes AS c
JOIN municipio AS m ON c.municipioid = m.id
JOIN departamento AS d ON m.depid = d.id
JOIN pais AS p ON d.paisid = p.id;
```
...
SELECT e.nombre
FROM empleados AS e
WHERE (SELECT AVG(e.salario)
      FROM empleados AS e
      WHERE e.puesto = 'Supervisor') < e.salario;
```
...
SELECT e.puesto
FROM empleados AS e
JOIN (SELECT e.puesto AS puesto,MAX(e.salario)
       FROM empleados AS e
       WHERE tn.salario > AVG(salario)
      ) AS tn ON e.puesto = tn.puesto;
```
